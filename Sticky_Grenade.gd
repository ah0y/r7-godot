...
func _ready():
	...
	explosion_particiles = $Explosion 
	
	explosion_particiles.emitting = false 
	explosion_particiles.one_shot = true
...
func _process(delta):
	...
	if grenade_timer < GRENADE_TIME:
		...
		if explosion_wait_timer <= 0:
			explosion_particiles.emitting = true
